...
describe("stake test", async function () {
    ...
    it("deposit exact min amount for ERC20 should pass (>=)", async () => {
        const proxyAddress = await stakeProxyContract.getAddress()
        // transfer exact min to user2 and deposit to ERC20 pool pid=1
        await erc20Contract.connect(admin).transfer(user2.address, ethers.parseEther("1"))
        await erc20Contract.connect(user2).approve(proxyAddress, ethers.parseEther("1"))
        await stakeProxyContract.connect(user2).deposit(1, ethers.parseEther("1"))
        const user2StakeErc20 = await stakeProxyContract.stakingBalance(1, user2.address)
        expect(user2StakeErc20).to.eq(BigInt(1E18))
    })
    ...
    it("global pause blocks core actions", async () => {
        await stakeProxyContract.connect(admin).pause()
        await expect(stakeProxyContract.connect(user1).depositETH({ value: ethers.parseEther("1") })).to.be.revertedWithCustomError(stakeProxyContract, "EnforcedPause")
        await expect(stakeProxyContract.connect(user1).deposit(1, ethers.parseEther("1"))).to.be.revertedWithCustomError(stakeProxyContract, "EnforcedPause")
        await expect(stakeProxyContract.connect(user1).unstake(0, ethers.parseEther("1"))).to.be.revertedWithCustomError(stakeProxyContract, "EnforcedPause")
        await expect(stakeProxyContract.connect(user1).withdraw(0)).to.be.revertedWithCustomError(stakeProxyContract, "EnforcedPause")
        await expect(stakeProxyContract.connect(user1).claim(0)).to.be.revertedWithCustomError(stakeProxyContract, "EnforcedPause")
